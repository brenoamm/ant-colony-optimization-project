...
my_data = pd.read_csv('../../../data_aggregation/TSP/HighLevel/Musket_1,3,5-12_average.csv', delimiter=',', header=None)
breno_data = pd.read_csv('../../../data_aggregation/TSP/HighLevel/Musket_TSP_average_ordererd.csv', delimiter=';')
print breno_data
#define size
...
# Data Breno
Breno1024 = breno_data.iloc[0].astype(float)
print Breno1024
Breno2048 = breno_data.iloc[1].astype(float)
Breno4096 = breno_data.iloc[2].astype(float)
Breno8192 = breno_data.iloc[3].astype(float)
...
for ax in fig.get_axes():
    ax.label_outer()
axes[0, 0].legend((BrenoBars_1024[0], MusketBars_1024[0]), ('Tesla V100', 'GTX'))
