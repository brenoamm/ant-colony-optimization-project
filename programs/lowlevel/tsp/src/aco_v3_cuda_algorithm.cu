...
__global__ void route_kernel(int n_cities, int* routes, double* c_phero, double* c_dist, double* d_probs, curandState* rand_states){
	...
	for (int i=0; i < n_cities-1; i++) {
		...
		for (int c = 0; c < n_cities; c++) {
			...
			if (cityi == c || vizited(ant_index, c,routes, n_cities, i)) {
				d_probs[ant_index*n_cities+c] = 0;
			}else{
				d_probs[ant_index*n_cities+c] = PHI(cityi, c, n_cities,  c_dist, c_phero, sum);
				count++;
			}
		}
...
//		printf("\n Distance %f Ant %i", rlength, ant_k);
	}
...
//	printf("\n CUDA-capable device count: %i", GPU_N);
	...
	while(iteration < n_iterations){
		...
		evaporation_kernel<<<n_cities, n_cities>>>(d_phero, d_ncities);

		pheromone_deposit_kernel<<<n_ants,(n_cities-1)>>>(d_nants, d_ncities, d_seq, d_phero, d_delta_phero, d_dist, d_routes_distance, d_bestRoute, d_best_sequence);

		cudaDeviceSynchronize();

		best_ant_kernel<<<1,1>>>(d_nants, d_ncities, d_seq, d_routes_distance, d_bestRoute, d_best_sequence);
