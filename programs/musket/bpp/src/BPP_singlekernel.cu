...
struct Packing_kernel_map_index_in_place_array_functor{
	...
	Packing_kernel_map_index_in_place_array_functor(const mkt::DArray<int>& _d_bins, const mkt::DArray<int>& _copy_bpp_items_quantity, const mkt::DArray<double>& _d_eta, const mkt::DArray<double>& _d_tau, const mkt::DArray<double>& _d_probabilities, const mkt::DArray<int>& _copy_bpp_items_weight, const mkt::DArray<double>& _d_phero, curandState* _d_rand_states_ind) : d_bins(_d_bins), copy_bpp_items_quantity(_copy_bpp_items_quantity), d_eta(_d_eta), d_tau(_d_tau), d_probabilities(_d_probabilities), copy_bpp_items_weight(_copy_bpp_items_weight), d_phero(_d_phero),  d_rand_states_ind(_d_rand_states_ind){}
	...
	auto operator()(int iindex, int y){
      ...
      for (int i = 0; ((i) < (itemtypess)); i++) {
        new_object_weight = copy_bpp_items_weight.get_global(i);
        object_quantity = copy_bpp_items_quantity.get_global(i);

//        if(ant_index == 0){
//            printf("\n Object %i : Weight %i Qty %i", i,  new_object_weight,object_quantity);
//        }
...
//       if(ant_index == 0){
//            printf("\n 1- Add Object %i : Weight %i", object_index,  object_weightmax);
//        }
      ...
      for (int i = 0; ((i) < ((itemcountt) - 1)); i++) {
        ...
        if (((possible_items_to_this_bin) > 0)) {
          ...
          int object_j = 0;
          double sum = d_probabilities.get_global((bpp_items_prefix));
          while (sum < random) {
            object_j = ((object_j) + 1);
            sum = (sum + d_probabilities.get_global(((bpp_items_prefix) + (object_j))));
          }
...
//       if(ant_index == 0){
//            printf("\n \t\t 2- Add Object %i : Weight %i", object_j,  weight_object_j);
//        }
          ...
          copy_bpp_items_quantity.set_global((bpp_items_prefix + object_index), (copy_bpp_items_quantity.get_global(bpp_items_prefix + object_index) - 1));
          d_bins.set_global(((((ant_index) * static_cast<int>((itemcountt))) + (i)) + 1), (object_index));

//            if(ant_index == 0){
//                    printf("\n 3- Add Object %i : Weight %i", object_index,  object_weightmax);
//                }
...
int mkt::reduce_max<int>(mkt::DArray<int>& a){
	...
	while(blocks > 1){
	  ...
	  mkt::kernel::reduce_max_call(blocks, d_odata, d_odata, threads_2, blocks_2, mkt::cuda_streams[0], 0);
	  blocks = blocks_2;
	}
...
//if(PALMA){
  std::string f60 = "/home/n/n_herr03/BPP/BPP/LowLevelProgram/source/bpp/Falkenauer_t60_00.txt";
  std::string p201 = "/home/n/n_herr03/BPP/BPP/LowLevelProgram/source/bpp/201_2500_NR_0.txt";
  std::string p402 = "/home/n/n_herr03/BPP/BPP/LowLevelProgram/source/bpp/402_10000_NR_0.txt";
  std::string p600 = "/home/n/n_herr03/BPP/BPP/LowLevelProgram/source/bpp/600_20000_NR_0.txt";
  std::string p801 = "/home/n/n_herr03/BPP//BPP/LowLevelProgram/source/bpp/801_40000_NR_0.txt";
  std::string p1002 = "/home/n/n_herr03/BPP/BPP/LowLevelProgram/source/bpp/1002_80000_NR_0.txt";
  //}
  ...
  d_bins.update_devices();
	...
	Packing_kernel_map_index_in_place_array_functor packing_kernel_map_index_in_place_array_functor{d_bins, copy_bpp_items_quantity, d_eta, d_tau, d_probabilities, copy_bpp_items_weight, d_phero, d_rand_states_ind};
  ...
  copybppitemsweight_map_index_in_place_array_functor.itemtypess = (itemtypes);
  mkt::map_index_in_place<int, Copybppitemsweight_map_index_in_place_array_functor>(copy_bpp_items_weight, copybppitemsweight_map_index_in_place_array_functor, n_threads, n_blocks);
  ...
  for(int iterate = 0; ((iterate) < (n_iterations)); iterate++){
    ...
    copybppitemsquantity_map_index_in_place_array_functor.itemtypess = (itemtypes);
    mkt::map_index_in_place<int, Copybppitemsquantity_map_index_in_place_array_functor>(copy_bpp_items_quantity, copybppitemsquantity_map_index_in_place_array_functor, n_threads, n_blocks);
	int maxobject = 0;
	auto copyend = std::chrono::high_resolution_clock::now();
        copytime += std::chrono::duration<double>(copyend-copystart).count();
	...
	auto packingend = std::chrono::high_resolution_clock::now();
	packing_time += std::chrono::duration<double>(packingend-p_start).count();

        auto eva_start = std::chrono::high_resolution_clock::now();
	...
	mkt::map_index_in_place<double, Evaporation_kernel_map_index_in_place_array_functor>(d_phero, evaporation_kernel_map_index_in_place_array_functor, itemtypes, itemtypes);
        auto eva_end = std::chrono::high_resolution_clock::now();
        evatime += std::chrono::duration<double>(eva_end-eva_start).count();

        auto min_start = std::chrono::high_resolution_clock::now();

	best_fitness = mkt::reduce_min<int>(d_fitness);
    	auto min_end = std::chrono::high_resolution_clock::now();
        mintime += std::chrono::duration<double>(min_end-min_start).count();
	...
	update_pheromones_kernel_map_index_in_place_array_functor.bin_capacity2 = (bin_capacity);
	 auto phero_start = std::chrono::high_resolution_clock::now();
    ...
    //(PRINT && !PALMA)?printf("\nBest Fitness (Number of bins used) Iteration %d: %d", iterate, best_fitness):printf("");
  	 auto phero_end = std::chrono::high_resolution_clock::now();
        pherotime += std::chrono::duration<double>(phero_end-phero_start).count();
  }
  ...
  if (PALMA) {
    printf(" %.5f;%.5f;%.5f;%.5f;%.5f;%.5f;", packing_time, copytime, evatime, mintime, pherotime, complete_seconds);
    printf(" %f;", packing_time/n_iterations);
