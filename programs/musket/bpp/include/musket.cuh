...
template<typename T, typename Functor>
void map_index_in_place(mkt::DArray<T>& a, Functor f, int threads, int blocks);
...
template<typename T, typename Functor>
void mkt::map_index_in_place(mkt::DArray<T>& a, Functor f, int threads, int blocks){
	...
	mkt::kernel::map_index_in_place<<< threads, blocks, smem_bytes, mkt::cuda_streams[gpu]>>>(devptr, gpu_elements, gpu_offset, f);
	}
}
